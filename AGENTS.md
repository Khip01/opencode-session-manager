...
## Release markdown must stay in sync (MANDATORY)

When a change affects what users see or do at install / uninstall /
upgrade time, BOTH must be updated in the same commit:

1. `.goreleaser.yml` `release.header` block (template that runs at
   every future release). Use `{{ .Version }}` for the version
   interpolation. Include install AND uninstall commands because
   both are first-class user flows.
2. The currently published release on GitHub (if the change is
   material to that version). Use `gh release edit <tag> --notes
   "..."` to update the body of an existing release without
   re-running goreleaser.

Triggers (non-exhaustive):
- New install path (script flag, go install path, brew formula, etc.)
- New uninstall mechanism (`opencode-sm uninstall` subcommand, etc.)
- CLI flag renamed or removed
- Default behavior change (e.g., watch mode default, db path)
- New required runtime dependency
- Migration note for users coming from previous version

This is mandatory. A published release with stale install/uninstall
instructions leads to user confusion and support burden. If you cut
a release and forgot to update the markdown, treat it as a release
bug and amend.
